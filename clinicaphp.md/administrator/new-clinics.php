...
// Получаем заявки на новые клиники
try {
    $requests = $db->select(
        "SELECT * FROM " . $db->table('new_clinic_requests') . "
...
<div class="space-y-4 sm:space-y-6 px-2 sm:px-0">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Заявки на новые клиники</h1>
    ...
    <?php if (empty($requests)): ?>
        <div class="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p class="text-gray-500">Заявок на новые клиники пока нет</p>
        </div>
    <?php else: ?>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Название клиники</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Контакты</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Город</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($requests as $request): ?>
                            <tr>
                                <td class="px-6 py-4"><?= e($request['clinic_name']) ?></td>
                                <td class="px-6 py-4">
                                    <div><?= e($request['contact_email'] ?? '') ?></div>
                                    <div class="text-xs text-gray-500"><?= e($request['contact_phone']) ?></div>
                                </td>
                                <td class="px-6 py-4"><?= e($request['city'] ?? '') ?></td>
                                <td class="px-6 py-4">
