...
export class DatabaseStorage implements IStorage {
  ...
  private transliterateRu(text: string): string {
  ...
  // Helper function to transliterate Romanian to Latin (normalize special chars)
  private transliterateRo(text: string): string {
    const map: Record<string, string> = {
      'ă': 'a', 'â': 'a', 'î': 'i', 'ș': 's', 'ț': 't',
      'Ă': 'A', 'Â': 'A', 'Î': 'I', 'Ș': 'S', 'Ț': 'T'
    };
  ...
  private createSlug(text: string, language: 'ru' | 'ro'): string {
    const transliterated = language === 'ru' ? this.transliterateRu(text) : this.transliterateRo(text);
  ...
  async createDistrict(district: InsertDistrict): Promise<District> {
    ...
    const districtWithSlugs = {
      ...district,
      slugRu: district.slugRu || this.createSlug(district.nameRu, 'ru'),
      slugRo: district.slugRo || this.createSlug(district.nameRo, 'ro')
    };

    const [newDistrict] = await db.insert(districts).values(districtWithSlugs).returning();
    await this.updateLastDataModification();
    return newDistrict;
  }
