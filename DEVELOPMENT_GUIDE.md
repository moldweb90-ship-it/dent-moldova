...
**Перед первым запуском проверьте `.env` файл в корне проекта:**

```bash
# Убедитесь, что в .env файле правильная строка подключения к development базе
...
**⚠️ Если в `.env` указана production база, клиники не будут загружаться!**
